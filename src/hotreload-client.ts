...
export interface ReloadStatus {
  ...
  functionReloads: number;
  configReloads: number;
  failedReloads: number;
  connectionStatus: 'connected' | 'disconnected' | 'reloading' | 'error';
  functionsCount: number;
  uptime: number;
}

export class HotReloadableClient extends EventEmitter {
  private config: Required<HotReloadConfig>;
  private isConnected: boolean = false;
  private isReloading: boolean = false;
  private functions: Map<string, RegisteredFunction> = new Map();
  private reconnectCount: number = 0;
  private functionReloads: number = 0;
  private configReloads: number = 0;
  private failedReloads: number = 0;
  ...
  constructor(config: HotReloadConfig = {}) {
    ...
    if (config.fileWatching) {
      this.config.fileWatching = { ...this.config.fileWatching, ...config.fileWatching };
    }
    if (config.tools) {
   ...
   * 注册函数
   */
  registerFunction(descriptor: FunctionDescriptor, handler: FunctionHandler): this {
    if (this.isReloading) {
    ...
    this.emit('functionRegistered', descriptor.id, descriptor);
    return this;
  }
   ...
   * 重新加载单个函数
   */
  async reloadFunction(functionId: string, descriptor: FunctionDescriptor, handler: FunctionHandler): Promise<this> {
    if (this.isReloading) {
   ...
   * 批量重载函数
   */
  async reloadFunctions(functions: Record<string, { descriptor: FunctionDescriptor; handler: FunctionHandler }>): Promise<this> {
    if (this.isReloading) {
      throw new Error('Reload operation already in progress');
    }
    ...
    const errors: Array<{ functionId: string; error: any }> = [];
   ...
   * 重载配置
   */
  async reloadConfig(newConfig: Partial<HotReloadConfig>): Promise<this> {
    console.log('🔄 Reloading client configuration');

    // 合并配置
    this.config = { ...this.config, ...newConfig } as Required<HotReloadConfig>;
   ...
   * 优雅关闭
   */
  async gracefulShutdown(timeout: number = this.config.gracefulShutdownTimeout): Promise<void> {
   ...
   * 设置Nodemon支持
   */
  private setupNodemonSupport(): void {
  ...
  private startFileWatching(): void {
    ...
    this.watcher.on('change', (filePath) => {
      ...
      if (filePath.endsWith('.js') || filePath.endsWith('.ts')) {
        this.handleJSFileChange(filePath);
   ...
   * 处理JS/TS文件变更
   */
  private handleJSFileChange(filePath: string): void {
    if (this.config.tools.moduleReload) {
      // 清除模块缓存
      ...
      this.emit('moduleReloaded', filePath);
    }
  }
  ...
  private handleConfigFileChange(filePath: string): void {
    try {
      ...
      this.emit('configFileChanged', filePath);
    } catch (error) {
      console.error('❌ Failed to handle config file change:', error);
    }
  }
   ...
   * 清除模块缓存
   */
  private clearModuleCache(functionId: string): void {
    // 简单的缓存清除策略
   ...
   * 注册所有函数到Agent
   */
  private async registerAllFunctions(): Promise<void> {
 ...
 * 工厂函数 - 创建热重载客户端
 */
export function createHotReloadClient(config?: HotReloadConfig): HotReloadableClient {
  return new HotReloadableClient(config);
}

/**
 * 导出默认类
 */
export default HotReloadableClient;